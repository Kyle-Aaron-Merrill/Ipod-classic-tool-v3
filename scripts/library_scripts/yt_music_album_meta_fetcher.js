...
async function ensureChromiumInstalled() {
    ...
    try {
        ...
        const verifyOptions = getPuppeteerLaunchOptions('yt-album-install-verify');
...
async function captureYoutubeMusicData(targetUrl, retryCount = 0) {
    ...
    try {
        ...
        const launchOptions = getPuppeteerLaunchOptions('yt-album');
        ...
        console.error(`✅ Captured HTML from YouTube Music`);
    } catch (error) {
 ...
 */
function extractYoutubeMusicMetadata(html, targetUrl, manifestPath, resolvedUrl = null) {
    ...
    if (artistLinks.length > 0) {
        ...
        console.error(`📝 Detected ${allArtists.length} artist(s): ${primaryArtist}`);
    }
    ...
    if (!primaryArtist) {
        ...
        if (parts.length >= 2) {
            ...
            if (!/^\d{4}$/.test(possibleArtist)) {
                ...
                console.error(`📝 Extracted artist from subtitle: ${primaryArtist}`);
            }
        }
    }
    ...
    const finalMetadata = {
        ...
        // Include resolved URL if provided (from browse link resolution)
        if (resolvedUrl) {
            finalMetadata.resolved_url = resolvedUrl;
            console.error(`[YouTube Music] Saving resolved URL to manifest: ${resolvedUrl}`);
        }
...
export async function getYoutubeMusicAlbumMeta(adamId, manifestPath, resolvedUrl = null) {
    ...
    try {
        ...
        extractYoutubeMusicMetadata(htmlContent, targetUrl, manifestPath, resolvedUrl);
    } catch (err) {
